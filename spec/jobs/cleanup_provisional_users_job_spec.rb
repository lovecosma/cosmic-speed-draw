...
RSpec.describe CleanupProvisionalUsersJob, type: :job do
  describe "#perform" do
    ...
    it "destroys provisional users older than 7 days with no drawings" do
      user = create(:provisional_user, created_at: cutoff - 1.second)
      described_class.perform_now
      expect(ProvisionalUser.exists?(user.id)).to be false
    end

    it "preserves provisional users newer than 7 days" do
      user = create(:provisional_user, created_at: cutoff + 1.second)
      described_class.perform_now
      expect(ProvisionalUser.exists?(user.id)).to be true
    end

    it "preserves provisional users older than 7 days that have drawings" do
      user = create(:provisional_user, created_at: cutoff - 1.second)
    ...
    it "destroys only eligible provisional users" do
      eligible    = create(:provisional_user, created_at: cutoff - 1.second)
      has_drawing = create(:provisional_user, created_at: cutoff - 1.second)
      too_recent  = create(:provisional_user, created_at: cutoff + 1.second)
      create(:drawing, :provisional, provisional_user: has_drawing)

      described_class.perform_now

      expect(ProvisionalUser.exists?(eligible.id)).to be false
      expect(ProvisionalUser.exists?(has_drawing.id)).to be true
