...
test.describe("provisional session", () => {
  test("creates a new provisional session on first visit", async ({
    request,
  }) => {
    const res = await request.post("/api/provisional_sessions");
  ...
  test("bootstraps a provisional user in a cold browser session", async ({
    page,
  }, testInfo) => {
  ...
  test("reuses the same provisional user when the JWT cookie is present", async ({
    request,
  }) => {
    const first = await request.post("/api/provisional_sessions");
    const firstBody = await first.json();
    const firstJwt = (first.headers()["set-cookie"] ?? "").match(
      /jwt_token=([^;]+)/,
    )?.[1];
    ...
    const secondJwt = (second.headers()["set-cookie"] ?? "").match(
      /jwt_token=([^;]+)/,
    )?.[1];
    expect(secondBody.user.id).toBe(firstBody.user.id);
    expect(secondJwt).toBeTruthy();
    expect(secondJwt).not.toBe(firstJwt);
  });

  test("creates a fresh provisional user when no cookie is present", async ({
    request,
  }, testInfo) => {
    const first = await request.post("/api/provisional_sessions");
    const firstBody = await first.json();
  ...
  test("falls back to a fresh provisional session when the browser has an invalid JWT", async ({
    page,
  }, testInfo) => {
    const baseURL = testInfo.project.use.baseURL;
  ...
  test("provisional JWT is rejected by authenticated endpoints", async ({
    request,
  }) => {
...
test.describe("provisional user → real user migration on sign up", () => {
  test("sign up migrates the provisional session to the new account", async ({
    page,
  }, testInfo) => {
    const baseURL = testInfo.project.use.baseURL;
  ...
  test("sign up replaces the provisional session with a persistent real session", async ({
    page,
    request,
  }, testInfo) => {
    const baseURL = testInfo.project.use.baseURL;
...
test.describe("provisional user → real user migration on sign in", () => {
  test("sign in migrates the provisional session to the existing account", async ({
    page,
    request,
  }, testInfo) => {
    const baseURL = testInfo.project.use.baseURL;
  ...
  test("sign in replaces the provisional session with a persistent real session", async ({
    page,
    request,
  }, testInfo) => {
    const baseURL = testInfo.project.use.baseURL;
...
test.describe("real user sign out after provisional upgrade", () => {
  test("sign out returns the browser to a fresh provisional session", async ({
    page,
    request,
  }, testInfo) => {
    const baseURL = testInfo.project.use.baseURL;
